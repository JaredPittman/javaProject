...
const ListBooks = () => {
  ...
  const handleClick = async () => {
    ...
    myHeaders.append(
      "Cookie",
      "connect.sid=s%3Aw1qerQho4F9cB23w-dil1kPjFJMMtaHB.6%2BXqphpaSqfeS8MUTSxZYAwXFkjfN3tcbpsMt2Q%2FV9w"
    );
  ...
  const handleUpdate = async (book) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/v1/books/${book.isbn}?isbn=` +
          book.isbn +
          "&title=" +
          book.title +
          "&editionNumber=" +
            ...
            (book, index) => (
              ...
              (
                <Tr key={index}>
                  ...
                  <Td>
                    {book.authors[0].authorFirstName +
                      " " +
                      book.authors[0].authorLastName}
                  </Td>
                  <Td>
...
const ListAuthors = () => {
  ...
  const handleUpdate = async (author) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/v1/authors/${author.authorID}?author_id=` +
          author.authorID +
          "&firstName=" +
  ...
  const handleAddAuthor = async (e, newAuthor) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8080/api/v1/authors?firstName=" +
          newAuthorFirstName +
          "&lastName=" +
          newAuthorLastName,
        requestOptions
          ...
          {authors.map((author, index) => (
            <Tr key={author.authorID}>
              ...
              <Td>
                <Editable
                  onChange={(value) => {
                    author.authorFirstName = value;
                  }}
                  defaultValue={author.authorFirstName}
                >
                  <EditablePreview />
                  <EditableInput />
                </Editable>
              ...
              <Td>
                <Editable
                  onChange={(value) => {
                    author.authorLastName = value;
                  }}
                  defaultValue={author.authorLastName}
                >
                  <EditablePreview />
                  <EditableInput />
                </Editable>
...
const BookByID = () => {
  ...
  const handleClick = async (selectedBookID) => {
    const myHeaders = new Headers();
    myHeaders.append(
      "Cookie",
      "connect.sid=s%3Aw1qerQho4F9cB23w-dil1kPjFJMMtaHB.6%2BXqphpaSqfeS8MUTSxZYAwXFkjfN3tcbpsMt2Q%2FV9w"
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/v1/books/${selectedBookID}`,
        requestOptions
      );
      const result = await response.json();
      setBook(result);
  ...
  return (
    <>
      <Input id='singleBookId' placeholder='Book ID' />
      <Button
        onClick={async () => {
          const selectedBookID = document.getElementById("singleBookId").value;
          console.log(selectedBookID);
          await handleClick(selectedBookID);
          console.log(book); // This will log the updated book state
        }}
      >
        Get Book
      </Button>
      <FormControl>
        ...
        <Input value={book?.edNumber} />
        <FormLabel>CopyRight</FormLabel>
        <Input value={book?.copyright} />
      </FormControl>
    </>
  );
};
...
const AuthorByID = () => {
  ...
  return (
    <>
      <Input id='singleAuthorId' placeholder='Author ID' />
      <Button
        onClick={async () => {
          const authorId = document.getElementById("singleAuthorId").value;
          console.log(authorId);
          await handleClick(authorId);
          console.log(authors); // This will log the updated authors state
        }}
      >
        Get Author
      </Button>
      <FormControl>
        ...
        <Input value={authors?.authorID} />
        <FormLabel>First Name</FormLabel>
        <Input value={authors?.authorFirstName} />
        <FormLabel>Last Name</FormLabel>
        <Input value={authors?.authorLastName} />
      </FormControl>
    </>
  );
};
